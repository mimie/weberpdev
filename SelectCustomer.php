...
/* $Id: SelectCustomer.php 5602 2012-08-19 07:04:01Z daintree $*/
...
if (isset($_POST['Search'])
	...
	if (($_POST['Keywords'] == '')
		...
		$_POST['MemberId'] = trim($_POST['MemberId']);
		$SQL = "SELECT debtorsmaster.debtorno,
						debtorsmaster.name,
					        debtorsmaster.memberid,
						debtorsmaster.address1,
					...
					AND debtorsmaster.memberid " . LIKE . " '%" . $_POST['MemberId'] . "%'
...
if ($_SESSION['CustomerID'] != '' AND !isset($_POST['Search']) AND !isset($_POST['CSV'])) {
	...
	echo '<p class="page_title_text"><img src="' . $rootpath . '/css/' . $theme . '/images/customer.png" title="' . _('Customer') . '" alt="" />' . ' ' . _('Customer') . ' : ' . $_SESSION['CustomerID'] . ' - ' . $CustomerName . ' - ' . $PhoneNo . _(' has been selected') . '<br>(Member ID: '.$MemberID.')</p>';
...
echo '<td><font size="3"><b>' . _('OR') . '</b></font></td>
                <td>' . _('Enter Member ID') . ':</td><td>';
if (isset($_POST['MemberId'])) {
        echo '<input type="text" name="MemberId" value="' . $_POST['MemberId'] . '" size="15" maxlength="18" />';
} else {
        echo '<input type="Text" name="MemberId" size="15" maxlength="18" />';
}
#noi-------
echo '</table><br />';
