  ...
  */
	...
	// it is a supplier payment by cheque and haven't printed yet so print cheque
		...
		echo '<br />
			...
			<br>
			<a href="' . $rootpath . '/print/printCV.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '&amp;amount=' . $_SESSION['PaymentDetail' . $identifier]->Amount.'&amp;Datepaid='. $_SESSION['PaymentDetail' . $identifier]->DatePaid .'&amp;PayTo='. $_POST['Narrative'].'&amp;UserName='.$Uname.'&amp;totalAmount='.$TotalAmount .'&amp;suppName='.$_GET['suppName'].'&amp;suppAdd='.$_SESSION['PaymentDetail' . $identifier]->Address1.'&amp;voucherNumber="'.$_POST['VouchNum'].'" target="_blank">' . _('Print CV') . '</a>

			<br />
			<br />';

/*<a href="' . $rootpath . '/print/printCV.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '&amp;amount=' . $_SESSION['PaymentDetail' . $identifier]->Amount.'&amp;Datepaid='. $_SESSION['PaymentDetail' . $identifier]->DatePaid .'&amp;PayTo='. $_SESSION['PaymentDetail' . $identifier]->SuppName.'&amp;UserName='.$Uname.'">' . _('Print CV') . '</a>
*/
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?identifier=' . $identifier . '&suppName='.$suppName) . '" method="post">';
echo '<div>';
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
...
if (!isset($_POST['VouchNum'])) {
        $_POST['VouchNum']='';
}

echo '<tr>
                <td>' . _('Voucher Number') . ':</td>
                <td><input type="text" name="VouchNum" maxlength="8" size="17" value="' . $_POST['VouchNum'] . '" /> </td>
        </tr>';
